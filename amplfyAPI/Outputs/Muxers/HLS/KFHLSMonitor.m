...
- (void) startMonitoringFolderPath:(NSString *)path delegate:(id<KFHLSUploaderDelegate>)delegate {
    dispatch_async(self.monitorQueue, ^{
        KFHLSUploader *hlsUploader = [[KFHLSUploader alloc] initWithDirectoryPath:path];
        hlsUploader.delegate = delegate;
        [self.hlsUploaders setObject:hlsUploader forKey:path];
    });
}

- (void) finishUploadingContentsAtFolderPath:(NSString*)path{
    dispatch_async(self.monitorQueue, ^{
        KFHLSUploader *hlsUploader = [self.hlsUploaders objectForKey:path];
        if (!hlsUploader) {
            hlsUploader = [[KFHLSUploader alloc] initWithDirectoryPath:path];
            [self.hlsUploaders setObject:hlsUploader forKey:path];
        }
...
- (void) uploader:(KFHLSUploader *)uploader uploadSpeed:(double)uploadSpeed numberOfQueuedSegments:(NSUInteger)numberOfQueuedSegments {
    DDLogInfo(@"[Background monitor] Uploaded segment @ %f KB/s, numberOfQueuedSegments %lu", uploadSpeed, (unsigned long)numberOfQueuedSegments);
}
